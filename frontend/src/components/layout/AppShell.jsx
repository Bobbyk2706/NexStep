      ...
      className={({ isActive }) =>
        mobile
          ...
          : `relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${
              isActive ? "bg-indigo-50 text-gold" : "text-slate-600 hover:bg-slate-50 hover:text-ink"
            }`
      }
    >
...
export default function AppShell() {
  ...
  return (
    <div className="min-h-screen bg-paper md:flex">
      {/* Desktop sidebar */}
      <aside className="hidden w-64 shrink-0 flex-col border-r border-slate-200/70 bg-surface px-4 py-6 md:flex">
      ...
      {/* Mobile bottom bar */}
      <nav className="fixed inset-x-0 bottom-0 z-30 flex border-t border-slate-200 bg-surface/95 backdrop-blur md:hidden">
        {NAV_ITEMS.map((item) => (
